...
function parseLocalDate(d: string): Date {
  // Already a full ISO string — parse as-is
  if (d.includes('T') || d.includes('Z')) return new Date(d);
  // Date-only YYYY-MM-DD — force local midnight to avoid UTC offset shift
...
export function formatBookingDateShort(dateStr: string): string {
  const d = safeDate(dateStr);
  if (!d) return dateStr;
